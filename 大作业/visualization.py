...
# 尝试多种常见中文字体，按优先级顺序
_CN_FONT_NAMES = [
    "Microsoft YaHei",  # 微软雅黑 (Windows) - 优先使用
    "SimHei",        # 黑体 (Windows)
    "Noto Sans SC",  # 思源黑体
    "STXihei",       # 华文细黑
...
_CN_FONT = None
for f in _CN_FONT_NAMES:
    try:
        _font_path = matplotlib.font_manager.findfont(f, fallback_to_default=False)
        if _font_path and f.lower() in _font_path.lower():
            _CN_FONT = f
            break
        # 有时 findfont 不抛异常但返回了默认字体
    except Exception:
        continue
...
if _CN_FONT:
    ...
    print(f"[可视化] 使用中文字体: {_CN_FONT} -> {matplotlib.font_manager.findfont(_CN_FONT)}")
else:
    plt.rcParams["font.sans-serif"] = ["DejaVu Sans"]
    print("[可视化] 警告: 未找到中文字体，中文将无法正常显示")
...
def plot_wordcloud(df: pd.DataFrame, save: bool = True):
    ...
    # 查找中文字体路径用于词云
    _wc_font_path = None
    for _fname in ["SimHei", "Microsoft YaHei", "STXihei", "Noto Sans SC", "Noto Sans CJK SC"]:
        try:
